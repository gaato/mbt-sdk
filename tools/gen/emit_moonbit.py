...
def _external_union(declaration: ExternalUnion) -> str:
    lines = _doc(declaration.description, f"Generated externally tagged union {declaration.name}.")
    lines.append(f"pub(all) enum {declaration.name} {{")
    for variant in declaration.variants:
    ...
    lines.append(f"pub extend {declaration.name} with @json.FromJson::{{from_json}}")
    lines.extend(["///|", f"pub impl @json.FromJson for {declaration.name} with fn from_json(value, path) {{", "  match value {"])
    for variant in declaration.variants:
        if not variant.type:
            lines.append(f"    String({_moon_string(variant.tag)}) => {variant.name}")
    lines.append("    Object(obj) => {")
    lines.append('      if obj.length() != 1 { raise @json.JsonDecodeError((path, "expected exactly one enum tag")) }')
    for variant in declaration.variants:
        if variant.type:
            tag = _moon_string(variant.tag)
            lines.append(f"      if obj.get({tag}) is Some(payload) {{ return {variant.name}(@json.from_json(payload, path=path.add_key({tag}))) }}")
    lines.extend(["      UnknownValue(value)", "    }", "    String(_) => UnknownValue(value)", '    _ => raise @json.JsonDecodeError((path, "expected string or single-key object enum"))', "  }", "}"])
    return "\n".join(lines)
...
def emit(ir: IR) -> dict[str, str]:
    ...
    for declaration in ir.declarations:
        ...
        elif isinstance(declaration, ExternalUnion):
            declarations.append(_external_union(declaration))
...
fn json_array_matches(
  ...
  match value {
    ...
    imports = ['  "gaato/http",'] if "@http." in body else []
    if "@runtime." in body:
